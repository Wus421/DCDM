...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_num_atom', type=int,
                        default=3)
    parser.add_argument('--outdir', type=str, default='./outputs_frag')
    ...
    while len(pool.finished) < config.sample.num_mols:
        ...
        batch_holder = make_data_placeholder_frag(n_graphs=n_graphs, min_num_atom=args.min_num_atom, start_linker=start_linker, data=data, device=args.device, max_size=max_size)
        ...
        # inference
        outputs = model.sample_diffusion_frag(
            n_graphs=n_graphs,
        ...
        for i_mol, output_mol in enumerate(output_list):
            ...
            # print(edge_type,mol_info['bond_type'])
            # exit()
            # edge_type = torch.tensor(mol_info['bond_type']).to(frag_mask_batch_edge.device)
            ...
            # print(edge_type[frag_mask_batch_edge[batch_halfedge==0]],data.halfedge_type)
            try:
            ...
            if '.' in smiles:
                ...
                if p_save_traj <  config.sample.save_traj_prob:
                    ...
                    for t in range(len(traj_info)):
                        try:
                            mol_traj.append(reconstruct_from_generated_with_frag(traj_info[t], False, add_edge=add_edge))
                        except MolReconsError:
