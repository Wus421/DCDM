...
class SimilarityAnalysis_Protein:
    def __init__(self, path) -> None:
        ...
        self.rdmol = Chem.RemoveHs(rdmol)
    ...
    def get_sim_with_val(self, mols, parallel=False):
        mol_finger = [Chem.RDKFingerprint(mol) for mol in mols]
        val_finger = [Chem.RDKFingerprint(self.rdmol)]
        finger_pair = list(itertools.product(mol_finger, val_finger))
        if not parallel:
            similarity_list = []
            similarity_lists = []
            for fg1, fg2 in tqdm(finger_pair, desc='Calculate similarity with val'):
                similarity_list.append(get_similarity((fg1, fg2)))
                similarity_lists.append({'sim_with_val':get_similarity((fg1, fg2))})
        else:
            with Pool(102) as pool:
                similarity_lists = list(tqdm(pool.imap(get_similarity, finger_pair), 
                                            total=len(mol_finger)*len(val_finger)))
                
        # calculate the max similarity of each mol with train data
        similarity_max = np.reshape(similarity_list, (len(mols), -1)).max(axis=1)
        # return np.mean(similarity_max)
        return np.mean(similarity_max), similarity_lists
    def get_sim_with_val_ecfp4(self, mols, parallel=False):
        mol_finger = [AllChem.GetMorganFingerprint(mol,2) for mol in mols]
        val_finger = [AllChem.GetMorganFingerprint(self.rdmol,2)]
        finger_pair = list(itertools.product(mol_finger, val_finger))
        if not parallel:
            similarity_list = []
            similarity_lists = []
            for fg1, fg2 in tqdm(finger_pair, desc='Calculate similarity with val'):
                similarity_list.append(TanimotoSimilarity(fg1, fg2))
                similarity_lists.append({'sim_with_val_ecfp4':TanimotoSimilarity(fg1, fg2)})
        else:
            with Pool(102) as pool:
                similarity_lists = list(tqdm(pool.imap(get_similarity, finger_pair), 
                                            total=len(mol_finger)*len(val_finger)))
