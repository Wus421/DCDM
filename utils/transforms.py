...
def make_data_placeholder_frag_x(n_graphs, min_num_atom, start_linker, data, device=None, max_size=None, x_id=None):
    ...
    n_nodes_list = [i if (i - num_node_frag) > min_num_atom else num_node_frag + min_num_atom for i in nodes_dist.tolist()]
    batch_node = np.concatenate([np.full(n_nodes, i) for i, n_nodes in enumerate(n_nodes_list)])
    ...
    for i_mol, n_nodes in enumerate(n_nodes_list):
        ...
        for i in range(n_nodes-1):
            if i < num_node_frag:
                if i !=x_id:
                    halfedge_type1 = torch.full([halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)-data.halfedge_index[0][data.halfedge_index[0]==i].size(0)], 0)
                    halfedge_type.extend(torch.cat([data.halfedge_type[idx_start_type:idx_start_type+data.halfedge_index[0][data.halfedge_index[0]==i].size(0)],halfedge_type1]))
                    frag_mask1 = torch.cat(
                    [torch.ones(data.halfedge_index[0][data.halfedge_index[0]==i].size(0), dtype=torch.long), torch.ones(halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)-data.halfedge_index[0][data.halfedge_index[0]==i].size(0), dtype=torch.long)])
                    frag_mask.extend(frag_mask1)
                    idx_start_type = idx_start_type + data.halfedge_index[0][data.halfedge_index[0]==i].size(0)
                else:
                    halfedge_type1 = torch.cat([torch.full([1],1),torch.full([halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)-data.halfedge_index[0][data.halfedge_index[0]==i].size(0)-1], 0)])
                    halfedge_type.extend(torch.cat([data.halfedge_type[idx_start_type:idx_start_type+data.halfedge_index[0][data.halfedge_index[0]==i].size(0)],halfedge_type1]))
                    frag_mask1 = torch.cat(
                    [torch.ones(data.halfedge_index[0][data.halfedge_index[0]==i].size(0)+1, dtype=torch.long), torch.ones(halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)-data.halfedge_index[0][data.halfedge_index[0]==i].size(0)-1, dtype=torch.long)])
                    frag_mask.extend(frag_mask1)
                    idx_start_type = idx_start_type + data.halfedge_index[0][data.halfedge_index[0]==i].size(0)
            else:
                frag_mask1 = torch.zeros(halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)-data.halfedge_index[0][data.halfedge_index[0]==i].size(0), dtype=torch.long)
                frag_mask.extend(frag_mask1)
                halfedge_type1 = torch.full([halfedge_index_this_mol[0][halfedge_index_this_mol[0]==i].size(0)], 0)
                halfedge_type.extend(halfedge_type1)
