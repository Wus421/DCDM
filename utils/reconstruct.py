...
def reconstruct_from_generated_with_frag(mol_info, check_validity=True, add_edge=None):
        ...
        if not fixed:
            try:
                ...
                if 'Unkekulized' in err.args[0]:
                    ...
                    #     bond_type += 2*[b_type]
                    # print(bond_type)
                    mol, fixed = fix_aromatic_frag(mol)

        # valence error for N 
        if not fixed:
            mol, fixed = fix_valence_frag(mol)
            
        if not fixed:
            mol, fixed = fix_aromatic_frag(mol, True)
        try:
            Chem.SanitizeMol(deepcopy(mol))
            Chem.Kekulize(deepcopy(mol))
        except Exception as e:
...
def fix_valence(mol):
    ...
    while True:
        ...
        if len(index) > 0:
            ...
            # mol.GetAtomWithIdx(int(index[0])).SetNumExplicitHs(2)
    return mol, fixed

def fix_valence_frag(mol):
    mol = deepcopy(mol)
    fixed = False
    cnt_loop = 0
    while True:
...
def fix_aromatic_frag(mol, strict=False):
    mol_orig = mol
    atomatic_list = [a.GetIdx() for a in mol.GetAromaticAtoms()]
