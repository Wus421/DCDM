...
class UniTransformerO2TwoUpdateGeneral_affinity(nn.Module):
    def __init__(self, edge_dim,num_blocks, num_layers, hidden_dim, use_gate, n_heads=1, k=32,
        ...
        num_gaussians = 16
        self.distance_expansion_moldiff = GaussianSmearingTime(start=0, stop=15, num_gaussians=num_gaussians)
        self.class2 = MLP(self.hidden_dim, 2, self.hidden_dim)
    def __repr__(self):
    ...
    def forward(self, h, x, mask_ligand, batch, h_edge_pert, edge_index, node_time, edge_time, return_all=False, fix_x=False):
        ...
        for b_idx, layer in enumerate(self.base_block):
            h, x = layer(h, x, edge_type, edge_index1, mask_ligand, e_w=e_w, fix_x=fix_x)
            
            all_x.append(x)
            all_h.append(h)
        final_h = self.class2(h)
        final_h = scatter_mean(final_h, batch, dim=0, dim_size=batch[-1]+1)
        outputs = {'x': x, 'h': final_h}
        if return_all:
    ...
    def _build_edges_dist(self, pos, edge_index):
        ...
        edge_dist = self.distance_expansion_moldiff(distance)
