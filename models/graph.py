...
class NodeEncoder(Module):
    ...
    def __init__(self, node_dim=256, edge_dim=64, key_dim=128, num_heads=4, 
        ...
        if dist_version == 'new':
            self.distance_expansion = GaussianSmearingTime(stop=cutoff, num_gaussians=20)
            self.edge_emb = Linear(self.additional_edge_feat+20, edge_dim)
        elif dist_version == 'old':
            self.distance_expansion = GaussianSmearingTime(stop=cutoff, num_gaussians=edge_dim-self.additional_edge_feat)
...
class NodeEdgeNet(Module):
    def __init__(self, node_dim, edge_dim, num_blocks, cutoff, use_gate, **kwargs):
        ...
        if 'start' not in kwargs:
            ...
            start = kwargs['start']
        self.distance_expansion = GaussianSmearingTime(start=start, stop=cutoff, num_gaussians=num_gaussians)
